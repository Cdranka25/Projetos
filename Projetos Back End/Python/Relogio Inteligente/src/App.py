...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.lbl_horario = ctk.CTkLabel(
            self.clock_frame, text="", font=("Helvetica", 36, "bold"), anchor="center"
        )
        self.lbl_horario.pack(pady=(10, 5), fill="x")

        self.lbl_data = ctk.CTkLabel(
            self.clock_frame, text="", font=("Helvetica", 14), anchor="center"
        )
        self.lbl_data.pack(pady=(0, 5), fill="x")

        self.lbl_localizacao = ctk.CTkLabel(
            self.clock_frame, text="", font=("Helvetica", 12), anchor="center"
        )
    ...
    def atualizar_relogio(self):
        ...
        if self.horario_atual:
            ...
            self.lbl_data.configure(
                text=f"{dia_semana}, {self.horario_atual.strftime('%d/%m/%Y')}"
            )
    ...
    def abrir_cronometro(self):
        ...
        self.label_cronometro = ctk.CTkLabel(
            frame, text="00:00:00:000", font=("Roboto Mono", 28)
        )
        ...
        self.btn_iniciar = ctk.CTkButton(
            btn_frame, text="Iniciar", command=self.iniciar_cronometro, width=80
        )
        ...
        self.btn_resetar = ctk.CTkButton(
            btn_frame, text="Resetar", command=self.resetar_cronometro, width=80
        )
    ...
    def abrir_temporizador(self):
        ...
        self.entrada_horas.configure(
            validatecommand=(self.register(self.validar_numerico), "%P")
        )
        ...
        self.entrada_minutos.configure(
            validatecommand=(self.register(self.validar_numerico), "%P")
        )
        ...
        self.entrada_segundos.configure(
            validatecommand=(self.register(self.validar_numerico), "%P")
        )
        ...
        self.btn_iniciar_temporizador = ctk.CTkButton(
            btn_frame, text="Iniciar", command=self.iniciar_temporizador, width=80
        )
        ...
        self.btn_resetar_temporizador = ctk.CTkButton(
            btn_frame, text="Resetar", command=self.resetar_temporizador, width=80
        )
        ...
        try:
            ...
            if tempo_total_ms <= 0:
                utils.mostrar_mensagem(
                    "Erro", "Digite um tempo maior que zero.", "warning"
                )
                return
    ...
    def tocar_alarme_default(self):
        try:
            ...
            caminho_audio = os.path.join(
                self.BASE_DIR, "..", "media", "aud", "temporizadorAlarm.mp3"
            )
    ...
    def abrir_despertador(self):
        ...
        for text, command in botoes:
            btn = ctk.CTkButton(
                btn_frame, text=text, command=command, width=110, height=30
            )
        ...
        scrollbar = ctk.CTkScrollbar(
            lista_frame, command=self.lista_despertadores.yview
        )
    ...
    def alterar_alarme(self):
        ...
        if len(selecionados) != 1:
            utils.mostrar_mensagem(
                "Aviso", "Selecione apenas um alarme para alterar.", "warning"
            )
            return
        index = selecionados[0]
        alarm_id = self._alarms_map[index]
        alarme = next(
            (a for a in self.despertador.list_alarms() if a["id"] == alarm_id), None
        )
        if alarme:
    ...
    def config_alarme(self, editar=False, alarme=None):
        ...
        self.entrada_hora_alarme = ctk.CTkEntry(
            hora_frame, textvariable=self.var_hora, width=60, validate="key"
        )
        self.entrada_hora_alarme.configure(
            validatecommand=(self.register(self.validar_numerico), "%P")
        )
        ...
        self.entrada_minuto_alarme = ctk.CTkEntry(
            minuto_frame, textvariable=self.var_minuto, width=60, validate="key"
        )
        self.entrada_minuto_alarme.configure(
            validatecommand=(self.register(self.validar_numerico), "%P")
        )
        ...
        if not self.sons_disponiveis:
            utils.mostrar_mensagem(
                "Erro", "Nenhum som de alarme encontrado na pasta 'media/aud'.", "error"
            )
            return
        ...
        ctk.CTkOptionMenu(
            self.janela_config, variable=self.var_audio, values=self.sons_disponiveis
        ).pack()
    ...
    def add_despertador(self, editar=False, alarme=None):
        try:
            ...
            if editar and alarme:
                self.despertador.remove_alarm(alarme.get("id"))

            if repetir and not dias_ints:
                utils.mostrar_mensagem(
                    "Erro",
                    "Selecione ao menos um dia para repetir o alarme.",
                    "warning",
                )
                return
            ...
            if alarm_id:
                ...
                utils.mostrar_mensagem(
                    "Erro", "Não foi possível adicionar o alarme", "error"
                )

        except ValueError:
            utils.mostrar_mensagem(
                "Erro", "Por favor, insira valores numéricos válidos.", "error"
            )
    ...
    def tocando_despertador(self, nome_audio):
        if self.alarme_tocando:
            return
        if (
            hasattr(self, "janela_tocandoDespertador")
            and self.janela_tocandoDespertador.winfo_exists()
        ):
        ...
        ctk.CTkButton(
            ...
            command=lambda: [
                self.parar_alarme(),
                self.janela_tocandoDespertador.destroy(),
            ],
            width=100,
            fg_color="#d9534f",
            hover_color="#c9302c",
        ...
        try:
            ...
            utils.mostrar_mensagem(
                "Erro", f"Não foi possível reproduzir o áudio: {str(e)}", "error"
            )
